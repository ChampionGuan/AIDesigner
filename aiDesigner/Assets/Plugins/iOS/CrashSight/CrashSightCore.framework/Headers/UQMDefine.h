...
public:
    ...
    UQMBaseRet() : retCode(UQMError::UNKNOWN), thirdCode(-1), thirdMsg("") {
        methodNameID = kUQMMethodNameUndefine;
    }

    // 构造函数对外使用，必须包含在外部调用，否则会 crash
    UQMBaseRet(int code) : retCode(code), thirdCode(-1), thirdMsg("") {
        methodNameID = kUQMMethodNameUndefine;
    }
...
public:
    ...
    UQMInnerBaseRet(int retCode, int thirdCode, UQMString thirdMsg);

    UQMInnerBaseRet(int retCode, UQMString retMsg, int thirdCode, UQMString thirdMsg);
#if UQM_PLATFORM_WINDOWS
#else
